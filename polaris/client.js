...
const {
  CommandManager, RobloxManager, Database, ErisExtensions, IPC
} = require("./util");

ErisExtensions(Eris); // Adds Extensions to Eris
const {
  withScope, captureException, Severity, captureEvent, captureMessage
} = require("@sentry/node");
  ...
  loadEvents () {
    this.on("ready", () => {
      ...
      this.editStatus("online", {
        name: `${this.guilds.size} servers | .help`,
        type: 3
      });
    });
    this.on("error", e => {
    ...
    if (process.env.NODE_ENV !== "production") {
      this.on("debug", m => {
        ...
        console.log(`DEBUG: `, util.inspect(m, { depth: 4 }));
      });
    }
