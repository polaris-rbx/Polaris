...
class User {
	...
	async updateInfo () {
		...
		try {
			...
			return {error: {message: 'Something happened that should not have happened. - User API', status: 500, robloxId: user.id, userName: user.username}};
		...
		try {
			...
			return {error: {message: 'Something happened that should not have happened. - status API', status: 500, robloxId: user.id, userName: user.username}};
			throw new Error(error);
		}
