...
class Group {
	...
	async getRole (userIdOrUserClass) {
		...
		try {
			...
			if (error.status === 404 || error.status === 400) return {error: {status: 404, message: 'User or group not found'}};
			if (error.status === 503) return {error: {status: 503, message: 'Service Unavailable - Roblox is down.'}};
			throw new Error(error);
		}
	}
	async updateInfo () {
		try {
			...
			if (error.status === 404) return {error: {status: 404, message: 'Group not found'}};
