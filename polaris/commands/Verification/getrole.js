...
class getRoleCommand extends BaseCommand {
  ...
  async execute (msg) {
    ...
    if (!rbxId) {
      ...
      return msg.channel.sendError(msg.author, {
        title: "Please link your account",
        description: `You need to link your account to get your roles.\nGet started with \`${prefix}linkaccount\`.`
      });
    }
  ...
  async giveRoles (settings, member, robloxId) {
    ...
    let bindToDelete;
    // binds
    for (const current of settings.binds) {
      if (member.guild.roles.get(current.role)) {
        const group = await this.client.roblox.getGroup(current.group);
        if (group.error) {
          return {
            error: {
              title: "HTTP Error",
    ...
    if (settings.subGroups && settings.subGroups.length > 0) {
      for (const sub of settings.subGroups) {
        await this.processGroup(rolesToGive, rolesToRemove, member, sub, robloxId);
      }
    }
    // ROLE GIVER
    for (const roleId of Object.keys(rolesToGive)) {
    ...
    // ROLE Remover
    for (const roleId of Object.keys(rolesToRemove)) {
    ...
    const newNick = await member.guild.updateNickname(settings, member, robloxId);
    if (newNick) {
      if (!newNick.error) {
        nickChanged = true;
      }
  ...
  async processGroup (rolesToGive, rolesToRemove, member, groupSettings, robloxId) {
    ...
    if (!group || group.error) {
      if (!group) {
        return {
          error: {
            title: "HTTP Error",
            description: "A HTTP Error has occurred. No group obtained."
          }
        };
      }
    ...
    if (groupSettings.ranksToRoles) {
      ...
      if (userRank.toLowerCase) {
        ...
        this.client.logError("userRank not defined", { userRank });
      }
    ...
    if (groupSettings.binds && groupSettings.binds.length > 0) {
      for (const current of groupSettings.binds) {
        const rank = await group.getRank(robloxId);
