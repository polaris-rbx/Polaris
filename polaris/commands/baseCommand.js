...
class BaseCommand {
  ...
  async process (message, args, prefix) {
    ...
    if (blacklist) {
      return message.channel.sendError(message.author, {
        title: "Blacklisted",
        description: `You are blacklisted from using Polaris. This is likely due to a violation of our Terms of service, or some other equally grievous action.\n**Reason: **${blacklist.reason ? blacklist.reason : "None provided."}\n**Date: **${blacklist.time}`
      });
    }
    ...
    if (message.channel.guild) {
      ...
      if (serverBlacklist) {
        await message.channel.sendError(message.author, {
          title: "Blacklisted!",
          description: `This server is blacklisted from using Polaris. This is likely due to a violation of our Terms of service, or some other equally grievous action.\n**Reason: **${serverBlacklist.reason ? serverBlacklist.reason : "None provided."}\n**Date: **${serverBlacklist.time}`,
          fields: [{
            name: "Leaving server",
            value: "I am now leaving the server. Please do not re-invite me."
          }]
        });
    ...
    } else if (this.permissions.length !== 0 || this.guildOnly) return message.channel.sendError(message.author, "This command **can only be ran in a server!**");
    ...
    if (message.author.id !== this.client.ownerId) {
      ...
      if (permissionFails.length !== 0) {
        return message.channel.sendError(message.author, {
          title: "Error",
          description: `The \`${this.name}\` command requires \`${permissionFails.join(", ")}\` permission(s)`
        });
      }
    }
