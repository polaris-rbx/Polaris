...
class BallCommand extends BaseCommand {
  ...
  async execute (msg) {
    const main = this.getRandomIntInclusive(1, 3);
    if (main === 1) {
  ...
  goodChoose () {
    const g = this.getRandomIntInclusive(0, good.length - 1);
    const text = good[g];
    return {
      title: "Magic 8ball",
  ...
  badChoose () {
    const g = this.getRandomIntInclusive(0, bad.length - 1);
    const text = bad[g];
    return {
      title: "Magic 8ball",
      description: `:8ball: ${text} :8ball:`
  ...
  noneChoose () {
    const g = this.getRandomIntInclusive(0, none.length - 1);
    const text = none[g];
    return {
      title: "Magic 8ball",
      description: `:8ball: ${text} :8ball:`
