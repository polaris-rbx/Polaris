...
class settingsCommand extends BaseCommand {
  ...
  async execute (msg, args) {
    ...
    try {
      do {
        const embed = this.makeMenu(menuName, current);

        const res = await this.sendPrompt(msg, embed, current.subs);
  ...
  makeMenu (menuName, menu) {
    ...
    if (menu.extra) {
      const extra = this.isFunction(menu.extra) ? menu.extra(this.guildSettings) : menu.extra;
      info = `${info}\n${extra}\n`;
    } else {
    ...
    const subMenus = Object.values(menu.subs);
    const subNames = Object.keys(menu.subs);

    for (const i in subMenus) {
  ...
  async sendPrompt (message, embed, options) {
    ...
    const fn = msg => (options[msg.content.toLowerCase()] && msg.author.id === message.author.id) || (msg.content.toLowerCase() === "cancel" && msg.author.id === message.author.id);
    const m = await message.channel.sendInfo(message.author, embed);
    if (!m) {
      // If message doesn't send for whatever reason
      COOLDOWN.delete(message.author.id);
    }

    let res = await message.channel.awaitMessages(fn, {
      maxMatches: 1,
      time: 30000
    });
...
async function setMainGroupId (msg, settings, client) {
  ...
  if (settings.mainGroup.id) {
    newGroupId = await msg.channel.prompt(msg.author, {
      title: "New group ID",
      description: `What is the new ID for your group?\nCurrently set to \`${settings.mainGroup.id}\`.`
    });
  } else {
    newGroupId = await msg.channel.prompt(msg.author, {
      title: "New group ID",
      description: "What is the ID of your group?"
    });
  }
  ...
  const newSetting = settings.mainGroup || {};
...
async function enableRTR (msg, settings, client) {
  ...
  const newMain = settings.mainGroup;
...
async function viewBinds (msg, settings, client) {
  ...
  const initialLength = settings.binds.length;

  if (!settings.binds || settings.binds.length <= 0) {
    return msg.channel.sendError(msg.author, {
      title: "No binds set",
      description: "You don't have any group binds set! Navigate to `binds` and say `add` to get started."
    });
  }
  const { binds } = settings;
  for (let count = 0; count < binds.length; count++) {
    const current = binds[count];

    const role = msg.channel.guild.roles.get(current.role);
    if (!role) {
      settings.binds.splice(count, 1);

      embed.fields.push({
        name: `Bind ${count + 1}`,
        value: `Deleted bind - Role removed`
      });
    } else {
      embed.fields.push({
        name: `Bind ${count + 1}`,
        value: `**Group ID:** ${current.group}\n**Group rank ID:** ${current.rank}\n**Role name:** ${role.name}\n**Exclusive:** ${current.exclusive}`,
        inline: true
      });
    }
  }
...
async function addBind (msg, settings, client) {
  ...
  // Variables
  let groupId = await msg.channel.prompt(msg.author, {
    title: "Please provide Group ID",
    description: "What is the Roblox group ID?"
  });
  if (!groupId) return;
  ...
  let rankId = await msg.channel.prompt(msg.author, {
    title: "Please provide Rank ID",
    description: "What is the Roblox Rank ID for this rank? It should be a number from 0 to 255."
  });
  if (!rankId) return;
  ...
  let roleName = await msg.channel.prompt(msg.author, {
    title: "Please provide role name or mention it",
    description: "What is the name of the Discord role? (Case sensitive), you can also mention the role."
  });
  if (!roleName) return;
  let roleId;
  ...
  if (!roleId) {
    ...
    const role = msg.channel.guild.roles.find(a => a.name === roleName);
    roleId = role.id;
  }

  let exclusive = await msg.channel.restrictedPrompt(msg.author, {
    title: "Is this bind Exclusive?",
    description: "Is this bind exclusive? If **Yes** then only this rank will get it.\nIf **no** all ranks above this rank will also recieve it."
  }, ["Yes", "No"]);
  if (!exclusive) return;
  ...
  for (let i = 0; i < binds.length; i++) {
    if (binds[i].rank === rankId && binds[i].role === roleId && binds[i].group === groupId && binds[i].exclusive === exclusive) {
      return msg.channel.sendError(msg.author, {
        ...
        description: "You already have a bind with those settings! If you would like to remove or edit it, remove the bind and add a new one."
      });
    }
  }
  ...
  const success = await client.db.updateSetting(msg.channel.guild.id, { binds });
...
async function rmvBind (msg, settings, client) {
  const embed = {
    ...
    footer: {
      ...
      text: `${msg.author.username} - Say an ID or "cancel"`
    }
  };
  ...
  for (let count = 0; count < settings.binds.length; count++) {
    const current = settings.binds[count];
  ...
  const fn = m => (parseInt(m.content, 10) > 0 && parseInt(m.content, 10) <= embed.fields.length && m.author.id === msg.author.id) || (m.content.toLowerCase() === "cancel" && m.author.id === msg.author.id);
  const m = await msg.channel.sendInfo(msg.author, embed);
  if (!m) return;
  // CUSTOM IMPLEMENTATION/USE OF MESSAGE COLLECTOR TO ALLOW FOR ERROR MESSAGES
  const collector = new client.MessageCollector(msg.channel, fn, {
    maxMatches: 1,
    time: 30000
  });

  collector.on("nonMatch", m => {
  ...
  collector.on("end", async (res, reason) => {
    if (reason === "time") {
      msg.channel.sendInfo(msg.author, {
        title: "Timed out",
        description: "Prompt timed out. You needed to say a number within 30 seconds!"
      });
      return;
    }
    res = res[0];
    ...
    if (res.content.toLowerCase() === "cancel") {
      res.channel.sendInfo(msg.author, {
        title: "Cancelled",
    ...
    if (!settings.binds[loc]) {
      msg.channel.sendError(msg.author, {
        ...
        description: "It looks like your selection wasn't valid but was accepted. Please try again."
      });
    ...
    if (success) {
      return msg.channel.sendSuccess(msg.author, {
        ...
        description: `Successfully removed the bind!`
      });
    }
    return msg.channel.sendError(msg.author, {
...
async function nicknameTemplates (msg, settings, client) {
  ...
  if (success) {
    return msg.channel.sendSuccess(msg.author, {
      title: "Successfully updated",
      description: `Successfully set the nickname template for this server to \`${newTemplate}\``
    });
  }
  return msg.channel.sendError(msg.author, {
    title: "Oops! Something went wrong.",
...
async function customNickname (msg, settings, client) {
  ...
  embed.fields.push({
    name: "Availible variables",
    value: m
  });
  embed.fields.push({
    name: "Notice",
    value: "Please note that nicknames that will be longer than 32 characters (discord max) will be shortened."
  });
  ...
  if (success) {
    return msg.channel.sendSuccess(msg.author, {
      title: "Successfully updated the template",
      description: `The new nickname template is \`${newTemplate}!\``
    });
  }
  return msg.channel.sendError(msg.author, {
    title: "Oops! Something went wrong.",
...
async function disableNickname (msg, settings, client) {
  ...
  if (success) {
    return msg.channel.sendSuccess(msg.author, {
      ...
      description: `Successfully disabled nickname management!`
    });
  }
  return msg.channel.sendError(msg.author, {
    title: "Oops! Something went wrong.",
...
async function changePrefix (msg, settings, client) {
  ...
  const res = await msg.channel.prompt(msg.author, {
    ...
    fields: [{
      name: "What would you like to set the prefix to?",
      value: "It can be anything! Up to 2 characters.\nPlease do not use a character such as \"a\" which will be used in normal conversation."
    }]
  }, true);
  if (!res) return;
  ...
  if (success) {
    return msg.channel.sendSuccess(msg.author, {
      title: "Successfully updated the prefix",
      description: `The new command prefix for this server is \`${res.content}\`! Users should use this new prefix to interact with Polaris.`
    });
  }
  return msg.channel.sendError(msg.author, {
    title: "Oops! Something went wrong.",
